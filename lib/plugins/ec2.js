...
function EC2(_tree){
	...
	this.cmds = [ 'info', 'ssh', 'cat', 'sshall', 'stats', 'cud', 'health', 'vols'];
}
...
function vols(args, callback){
	...
	/* Put some more detail from the instance object
		{ InstanceId: 'i-0aec868a92f7678af',
		  AvailabilityZone: 'us-east-1b',
		  LifecycleState: 'InService',
		  HealthStatus: 'Healthy',
		  LaunchConfigurationName: 'volpe-boobj34a-5-LaunchConfigurationSolr-NQPW7DVM7UNN',
			...
			if (err){
				...
				for(var j = 0; j < mappings.length; j++){
					if(mappings[j].DeviceName === '/dev/sdd'){
						has_vol = true;
						console.log(instance_id, mappings[j].Ebs.VolumeId);
					}
				}
