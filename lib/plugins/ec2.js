...
EC2.prototype.do_cmd = function(args, opts, callback){
	...
	switch(args[0]){
		...
		case 'syncall': syncall(args, opts, callback); break;
...
function sync(args, opts, callback){
	...
	if(tree.current.type !== 'Instance'){
		console.log('You can only use "sync" on instance nodes.');
		callback(); return;
	}
...
function syncall(args, opts, callback){

	if(!args[1] || !args[2]){
		console.log('Usage: syncall <local path> <remote path>');
		callback(); return;
	}
