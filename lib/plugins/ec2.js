...
function EC2(_tree){
	...
	this.cmds = [ 'info', 'ssh', 'cat', 'sshall', 'stats'];
}
...
function run_cmd_via_ssh(ip, command, args, callback){
	...
	var ssh = new ssh_client.SSH({
	    ...
	    port: 22,
	    identity_file: process.env.RT_KEY_PATH
	});
...
function ssh_into_instance(id, ip){
	...
	command += "\"echo '" + id + "' && ssh -o StrictHostKeyChecking=no -i " + process.env.RT_KEY_PATH;
