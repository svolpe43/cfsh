...
function sshfw(args, opts, callback){
	...
	if(node_path === ''){
		...
		get_ip(tree.current.aws_id, function(ip){
			ssh_forward(id, ip, forward_port, dest_port, proxy, callback);
		});
	}else{
...
function run_cmd_via_ssh(instance, args, proxy, callback){
	...
	if(proxy){
		ssh_params.option.ProxyCommand = 'ssh bastion -W %h:%p'
	}
...
function start_ssh(id, ip, proxy, callback){
	...
	var command;
	if(proxy){
		var command = "echo '" + id + "' && ssh -tt bastion ssh -o StrictHostKeyChecking=no " + settings.ssh_user + "@" + ip;
	}else{
...
function ssh_forward(id, ip, forward_port, dest_port, proxy, callback){
	...
	if(ip && dest_port && forward_port){
		...
		var command = "echo '" + id + "' && ssh -o StrictHostKeyChecking=no -L " + forward_port + ":localhost:" + dest_port + " " + settings.ssh_user + "@" + ip;
		var osa = settings.osa_ssh_command.replace(/{{{command}}}/, command);
		var ssh = exec(osa, function(){
