...
function parse_text(text){
	...
	switch(command[0]){
		...
		case "info": cfsh.info(command); break;
		case "mk": cfsh.mk(command); break;
		case "rm": cfsh.rm(command); break;
		case "up": cfsh.up(command); break;
...
Cfsh.prototype.info = function(command){

	var self = this;
	this.tree.info({}, function(){
		write_path(self.tree.current_path());
	});
}
...
Cfsh.prototype.up = function(command){

	var self = this;
	this.tree.get_node('/' + command[2], function(stack_node){

		if(stack_node){
			ops.up(command[1], stack_node.name, function(msg){
...
Cfsh.prototype.rm = function(command){

	var self = this;
	this.tree.get_node('/' + command[1], function(stack_node){

		if(stack_node){
			ops.rm(stack_node, function(msg){
